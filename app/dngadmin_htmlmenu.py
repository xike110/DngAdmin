...
def htmlmenu(request):
	...
	zd_list = dngadmin_common.dng_ziduan("htmlmenu") #获取对应表下所有字段值
	# ----------------------------------------------------------
	#   判断页面权限开始《《《 结束
	# ----------------------------------------------------------
	...
	return render(request,"dngadmin/htmlmenu.html",{
...
def htmlmenu_json(request):
	...
	dngred = models.route.objects.filter().order_by('-id')
...
def htmlmenu_added(request):  #新增
	...
	ok = '新增失败'
	off = False
	pwname =models.route.objects.filter(name_str=post2).only('name_str')
	pwurl = models.route.objects.filter(url_str=post3).only('url_str')
	...
	if added:
		...
		if not post2:
			...
			users = models.usergroup.objects.filter(gid_int=1).first()  # 读用户组权限
			ider = models.route.objects.filter().order_by('-uid_int').first()  # 读ID
			sortdb = models.route.objects.filter().order_by('-sort_int').first()  # 读排序
			...
			models.route.objects.create(uid_int=uid, name_str=post2, url_str=post3, model_str=post5,
										   superior_int=post6, sort_int=sort)  # 增加菜单表
			models.usergroup.objects.filter(gid_int=1).update(menu_text=qx_menu, added_text=qx_added,
...
def htmlmenu_delete(request):  #删除
	# ----------------------------------------------------------
	#   判断页面权限》》》 开始
	# ----------------------------------------------------------
	...
	if delete: #判断删除权限
		...
		if delete_id: #判断单个删除
			uxxg = models.route.objects.filter(id=delete_id).first()
			udhg = models.route.objects.filter(superior_int=uxxg.uid_int).first()
			if udhg:
				off = "下面还有二级菜单,不能删除"
				return HttpResponse(off)

			ok = models.route.objects.filter(id=delete_id).delete()
			...
			if "," in id :
				...
				for key in id:
					int(key)
					uxxg = models.route.objects.filter(id=key).first()
					udhg = models.route.objects.filter(superior_int=uxxg.uid_int).first()
					if udhg:
						off = "下面还有二级菜单,不能删除"
						return HttpResponse(off)

					ok = models.route.objects.filter(id=key).delete()
					if ok:
						ok = 'yes'
					else:
						ok = 'no'
				...
				uxxg = models.route.objects.filter(id=id).first()
				udhg = models.route.objects.filter(superior_int=uxxg.uid_int).first()
				if udhg:
					off = "下面还有二级菜单,不能删除"
					return HttpResponse(off)

				ok = models.route.objects.filter(id=id).delete()
				if ok:
					ok = 'yes'
				else:
					ok = 'no'
...
def htmlmenu_update(request):  #更新修改
	# ----------------------------------------------------------
	#   判断页面权限》》》 开始
	# ----------------------------------------------------------
	...
	if update:
		...
		if post1:
			models.route.objects.filter(id=post0).update(uid_int=post1)  # 菜单id
		if post2:
			models.route.objects.filter(id=post0).update(name_str=post2)  # 菜单名称
		if post3:
			models.route.objects.filter(id=post0).update(url_str=post3)  # 菜单URL
		if post4:
			models.route.objects.filter(id=post0).update(icon_str=post4)  # 菜单图标
		if post5:
			models.route.objects.filter(id=post0).update(model_str=post5)  # 菜单模型
		if post6:
			models.route.objects.filter(id=post0).update(superior_int=post6)  # 上级菜单
		if post7:
			models.route.objects.filter(id=post0).update(sort_int=post7)  # 菜单排序
		if post8:
			models.route.objects.filter(id=post0).update(integral_int=post8)  # 积分阈值
		if post9:
			models.route.objects.filter(id=post0).update(money_int=post9)  # 余额阈值
		if post10:
			models.route.objects.filter(id=post0).update(totalmoney_int=post10)  # 充值阈值
		if post11:
			models.route.objects.filter(id=post0).update(totalspend_int=post11)  # 消费阈值
		if post12:
			models.route.objects.filter(id=post0).update(spread_int=post12)  # 推广阈值

		if post13:
			if post13=='on':
				post13=True
				models.route.objects.filter(id=post0).update(display_bool=post13)  # 菜单显示
			elif post13=='off':
				post13 = False
				models.route.objects.filter(id=post0).update(display_bool=post13)  # 菜单显示
...
def htmlmenu_search(request):  #搜索
	...
	if post1:
		dngred = models.route.objects.filter(uid_int__contains=post1).order_by('-id')
	elif post1:
		dngred = models.route.objects.filter(uid_int__contains=post1).order_by('-id')
	elif post2:
		dngred = models.route.objects.filter(name_str__contains=post2).order_by('-id')
	elif post3:
		dngred = models.route.objects.filter(url_str__contains=post3).order_by('-id')
	elif post4:
		dngred = models.route.objects.filter(icon_str__contains=post4).order_by('-id')
	elif post5:
		dngred = models.route.objects.filter(model_str__contains=post5).order_by('-id')
	elif post6:
		dngred = models.route.objects.filter(superior_int__contains=post6).order_by('-id')
	elif post7:
		dngred = models.route.objects.filter(sort_int__contains=post7).order_by('-id')
	elif post8:
		dngred = models.route.objects.filter(integral_int__contains=post8).order_by('-id')
	elif post9:
		dngred = models.route.objects.filter(money_int__contains=post9).order_by('-id')
	elif post10:
		dngred = models.route.objects.filter(totalmoney_int__contains=post10).order_by('-id')
	elif post11:
		dngred = models.route.objects.filter(totalspend_int__contains=post11).order_by('-id')
	elif post12:
		dngred = models.route.objects.filter(spread_int__contains=post12).order_by('-id')
	elif post13:
		dngred = models.route.objects.filter(display_bool__contains=post13).order_by('-id')
