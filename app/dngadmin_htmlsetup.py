...
def htmlsetup(request):
	...
	zd_list = dngadmin_common.dng_ziduan("htmlsetup") #获取对应表下所有字段值

	biaodan = dngadmin_common.dng_htmlsetup()


	return render(request,"dngadmin/htmlsetup.html",{
...
def htmlsetup_post(request):
	...
	if update:
		if not post1:
			urlstr = parse.quote('首页标题不能为空')
			response = HttpResponseRedirect('/dngadmin/htmlsetup/?jinggao=' + urlstr)
			return response
		if not post2:
			urlstr = parse.quote('META关键词不能为空')
			response = HttpResponseRedirect('/dngadmin/htmlsetup/?jinggao=' + urlstr)
			return response
		if not post3:
			urlstr = parse.quote('META描述不能为空')
			response = HttpResponseRedirect('/dngadmin/htmlsetup/?jinggao=' + urlstr)
			return response
		...
		models.htmlsetup.objects.filter(id=1).update(title_str=post1, keywords_str=post2, description_str=post3,
													file_str =post4, statistics_text=post5, register_bool=post6,)  # 更新修改数据库
		urlstr = parse.quote('提交修改成功')
		response = HttpResponseRedirect('/dngadmin/htmlsetup/?yes=' + urlstr)
		return response
	else:
