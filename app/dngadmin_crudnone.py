...
def crudnone(request):
	...
	if api_post:
		if not dngadmin_common.dng_ziduan(api_post) :
			...
			response = HttpResponseRedirect('/dngadmin/crudnone/?jinggao=' + urlstr)
			return response
	...
	return render(request,"dngadmin/crudnone.html",{
...
def crudnone_post(request):
    ...
    # ⊙1数据库模型替换1⊙'''
        ...
        # ⊙2数据库模型替换2⊙'''
			...
			response = HttpResponseRedirect('/dngadmin/crudnone/?yes=' + urlstr)
			return response
