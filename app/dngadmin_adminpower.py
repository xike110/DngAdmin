...
def adminpower_menu(request): #访问权限
	...
	caidan = models.dngroute.objects.filter(superior_int=0,prove_bool=True).order_by('sort_int')  #查询主菜单
	...
	cdidss = models.dngroute.objects.filter(~Q(superior_int=0,prove_bool=True)).order_by('sort_int')    # 查询所有菜单ID
	looo =[]
	for sk in cdidss:
