...
def crudform(request):
	...
	if api_post:
		if not dngadmin_common.dng_ziduan(api_post) :
			...
			response = HttpResponseRedirect('/dngadmin/crudform/?jinggao=' + urlstr)
			return response
	...
	return render(request,"dngadmin/crudform.html",{
...
def crudform_post(request):
    ...
    # ⊙1数据库模型替换1⊙'''
        ...
        # ⊙2数据库模型替换2⊙'''
			...
			response = HttpResponseRedirect('/dngadmin/crudform/?yes=' + urlstr)
			return response
