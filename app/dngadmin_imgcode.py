...
def imgcode(request):
	...
	#zd_list = dngadmin_common.dng_ziduan("imgcode") #获取对应表下所有字段值

	#biaodan = dngadmin_common.dng_dnguser(uid=dnguser_uid)


	return render(request,"dngadmin/imgcode.html",{
...
def imgcode_post(request):
	...
	if update:
		if post1:
			...
			response = HttpResponseRedirect('/dngadmin/imgcode/?yes=' + urlstr)
			return response
		...
		response = HttpResponseRedirect('/dngadmin/imgcode/?jinggao=' + urlstr)
		return response
