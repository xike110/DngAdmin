...
def none(request):
	...
	return render(request,"dngadmin/none.html",{
...
def none_post(request):
	...
	zd_list = dngadmin_common.dng_ziduan("none") #获取对应表下所有字段值
	...
	if update:
		...
		response = HttpResponseRedirect('/dngadmin/none/?yes=' + urlstr)
		return response
		...
		response = HttpResponseRedirect('/dngadmin/none/?jinggao=' + urlstr)
		return response



# def none_api_json(request):  #api查询
#
# 	# ----------------------------------------------------------
# 	#   判断页面权限》》》 开始
# 	# ----------------------------------------------------------
...
# 	#   判断页面权限开始《《《 结束
# 	# ----------------------------------------------------------
# 	list_arry=models.none.objects.filter().order_by ("-id")
# 	gongju = models.none.objects.count()
#
# 	data = []
