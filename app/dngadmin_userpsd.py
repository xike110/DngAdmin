...
def userpsd(request):
	...
	#zd_list = dngadmin_common.dng_ziduan("userpsd") #获取对应表下所有字段值

	#biaodan = dngadmin_common.dng_dnguser(uid=dnguser_uid)


	return render(request,"dngadmin/userpsd.html",{
...
def userpsd_post(request):
	...
	if update:
		if not post1:
			urlstr = parse.quote('旧密码不能为空')
			response = HttpResponseRedirect('/dngadmin/userpsd/?jinggao=' + urlstr)
			return response
		if not post2:
			urlstr = parse.quote('新密码没有填写')
			response = HttpResponseRedirect('/dngadmin/userpsd/?jinggao=' + urlstr)
			return response
		if not post3:
			urlstr = parse.quote('重复新密码不能为空')
			response = HttpResponseRedirect('/dngadmin/userpsd/?jinggao=' + urlstr)
			return response

		post1 = hashlib.md5(post1.encode(encoding='UTF-8')).hexdigest()  # 密码MD5加密


		if biaodan.password_str==post1:

			if (post2 != post3):
				urlstr = parse.quote('新密码与重复密码不一致')
				response = HttpResponseRedirect('/dngadmin/userpsd/?jinggao=' + urlstr)
				return response
			elif (post2 == post3):
				post2 = hashlib.md5(post2.encode(encoding='UTF-8')).hexdigest()  # 密码MD5加密
				models.dnguser.objects.filter(uid_int=dnguser_uid).update(password_str=post2)  # 更新修改数据库
				if dngadmin_common.html_name(biaodan.username_str):  # 如何后台有相同用户,同步重置密码
					models.user.objects.filter(username_str=biaodan.username_str).update(password_str=post2)  # 更新修改数据库
				urlstr = parse.quote('提交修改成功')
				response = HttpResponseRedirect('/dngadmin/userpsd/?yes=' + urlstr)
				return response


		else:
			urlstr = parse.quote('旧密码不正确')
			response = HttpResponseRedirect('/dngadmin/userpsd/?jinggao=' + urlstr)
			return response
