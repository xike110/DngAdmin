...
def pluguser(request):
	...
	zd_list = dngadmin_common.dng_ziduan("pluguser")  # 获取对应表下所有字段值
	db_values_list = models.pluguser.objects.filter().values_list()  # 获取数据返回元组列表格式
	html_form = dngadmin_formcommon.form_form(zd_list, db_values_list)  # 获取表单前端组件组合


	return render(request,"dngadmin/pluguser.html",{
...
def pluguser_post(request):
	...
	if update:
		if post0:
			...
			urlstr = parse.quote('退出成功')
			response = HttpResponseRedirect('/dngadmin/pluguser/?yes=' + urlstr)
			return response

		else:


			urlstr = parse.quote('登录成功')
			response = HttpResponseRedirect('/dngadmin/pluguser/?yes=' + urlstr)
			return response

	else:
			urlstr = parse.quote('您没有修改权限')
			response = HttpResponseRedirect('/dngadmin/pluguser/?jinggao=' + urlstr)
			return response
