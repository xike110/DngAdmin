...
def adminuser(request):
	...
	groupall =	dngadmin_common.dng_groupall()
	...
	return render(request,"dngadmin/adminuser.html",{
...
def adminuser_json(request):
	...
	dngred = models.dnguser.objects.filter().order_by('-id')
	...
	for key in book_list:
		group  = dngadmin_common.dng_usergroup(gid=dngadmin_common.dng_dnguser(key.uid_int).group_int)
...
def adminuser_added(request):  #新增
	# ----------------------------------------------------------
	#   判断页面权限》》》 开始
	# ----------------------------------------------------------
	...
	ok = '新增失败'
	off = False
	pws =models.dnguser.objects.filter(username_str__contains=post2).only('username_str')
	if added:
		...
		if not post2:
			...
			user = models.dnguser.objects.filter().order_by('-uid_int').first()

			uid = int(user.uid_int)+1 #会员ID递增
			models.dnguser.objects.create(uid_int=uid,username_str=post2, password_str=post3_md5, group_int=post9,)
...
def adminuser_delete(request):  #删除
	# ----------------------------------------------------------
	#   判断页面权限》》》 开始
	# ----------------------------------------------------------
	...
	if delete: #判断删除权限
		...
		if delete_id: #判断单个删除
			if not dngadmin_common.dng_gmid(id=delete_id) == False:
				off = "超级管理员不能删除"
				return HttpResponse(off)

			ok = models.dnguser.objects.filter(id=delete_id).delete()
			...
			if "," in id :
				...
				for key in id:
					...
					if not dngadmin_common.dng_gmid(id=key) == False:
						off = "超级管理员不能删除"
						return HttpResponse(off)

					ok = models.dnguser.objects.filter(id=key).delete()
					if ok:
						ok = 'yes'
					else:
						ok = 'no'
				...
				if not dngadmin_common.dng_gmid(id=id) == False:
					off = "超级管理员不能删除"
					return HttpResponse(off)

				ok = models.dnguser.objects.filter(id=id).delete()
				if ok:
					ok = 'yes'
				else:
					ok = 'no'
...
def adminuser_update(request):  #更新修改
	# ----------------------------------------------------------
	#   判断页面权限》》》 开始
	# ----------------------------------------------------------
	...
	if update:
		...
		if post1:
			models.dnguser.objects.filter(id=post0).update(uid_int=post1)

		if post2:
			models.dnguser.objects.filter(id=post0).update(username_str=post2)

		if post3:
			models.dnguser.objects.filter(id=post0).update(password_str=post3)

		if post4:
			models.dnguser.objects.filter(id=post0).update(name_str=post4)

		if post5:

			models.dnguser.objects.filter(id=post0).update(gender_str=post5)

		if post6:
			models.dnguser.objects.filter(id=post0).update(introduce_str=post6)

		if post7:
			models.dnguser.objects.filter(id=post0).update(emall_str=post7)

		if post8:
			models.dnguser.objects.filter(id=post0).update(mobile_str=post8)

		if post9:
			if not dngadmin_common.dng_gmid(id=post0) == False:
				off = "超级管理员不能被-修改用户组"
				return HttpResponse(off)

			models.dnguser.objects.filter(id=post0).update(group_int=post9)

		if post10:
			models.dnguser.objects.filter(id=post0).update(rank_str=post10)

		if post12:
			models.dnguser.objects.filter(id=post0).update(money_int=post12)

		if post13:
			models.dnguser.objects.filter(id=post0).update(totalmoney_int=post13)

		if post14:
			models.dnguser.objects.filter(id=post0).update(totalspend_int=post14)

		if post15:
			models.dnguser.objects.filter(id=post0).update(integral_int=post15)

		if post16:
			models.dnguser.objects.filter(id=post0).update(spread_int=post16)
		...
		if post23:
			...
			models.dnguser.objects.filter(id=post0).update(frozen_bool=post23)


		if post25:
			models.dnguser.objects.filter(id=post0).update(vipime_time=post25)
		...
		if off:
			...
			models.dnguser.objects.filter(id=post0).update(update_time=shijian)
...
def adminuser_search(request):  #搜索
	...
	if   post1:
		...
		dngred = models.dnguser.objects.filter(uid_int__contains=post1).order_by('-id')

	elif post2:

		dngred = models.dnguser.objects.filter(username_str__contains=post2).order_by('-id')

	elif post3:
		dngred = models.dnguser.objects.filter(password_str__contains=post3).order_by('-id')

	elif post4:
		dngred = models.dnguser.objects.filter(name_str__contains=str(post4)).order_by('-id')

	elif post5:
		dngred = models.dnguser.objects.filter(gender_str__contains=post5).order_by('-id')

	elif post6:
		dngred = models.dnguser.objects.filter(introduce_str__contains=post6).order_by('-id')

	elif post7:
		dngred = models.dnguser.objects.filter(emall_str__contains=post7).order_by('-id')

	elif post8:
		dngred = models.dnguser.objects.filter(mobile_str__contains=post8).order_by('-id')

	elif post9:
		gp  = models.dngusergroup.objects.filter(gname_str__contains=post9).first()
		dngred = models.dnguser.objects.filter(group_int__contains=gp.gid_int).order_by('-id')

	elif post10:
		dngred = models.dnguser.objects.filter(rank_str__contains=post10).order_by('-id')

	elif post11:
		dngred = models.dnguser.objects.filter(gm_bool__contains=post11).order_by('-id')

	elif post12:
		dngred = models.dnguser.objects.filter(money_int__contains=post12).order_by('-id')

	elif post13:
		dngred = models.dnguser.objects.filter(totalmoney_int__contains=post13).order_by('-id')

	elif post14:
		dngred = models.dnguser.objects.filter(totalspend_int__contains=post14).order_by('-id')

	elif post15:
		dngred = models.dnguser.objects.filter(integral_int__contains=post15).order_by('-id')

	elif post16:
		dngred = models.dnguser.objects.filter(spread_int__contains=post16).order_by('-id')

	elif post17:
		dngred = models.dnguser.objects.filter(ip_str__contains=post17).order_by('-id')

	elif post18:
		dngred = models.dnguser.objects.filter(shebei_str__contains=post18).order_by('-id')

	elif post19:
		dngred = models.dnguser.objects.filter(cookie_str__contains=post19).order_by('-id')

	elif post20:
		dngred = models.dnguser.objects.filter(token_str__contains=post20).order_by('-id')

	elif post21:
		dngred = models.dnguser.objects.filter(days_int__contains=post21).order_by('-id')

	elif post22:
		dngred = models.dnguser.objects.filter(pwderror_int__contains=post22).order_by('-id')

	elif post23:
		dngred = models.dnguser.objects.filter(frozen_bool__contains=post23).order_by('-id')

	elif post24:
		dngred = models.dnguser.objects.filter(frozentime_str__contains=post24).order_by('-id')

	elif post25:
		dngred = models.dnguser.objects.filter(vipime_time__contains=post25).order_by('-id')

	elif post26:
		dngred = models.dnguser.objects.filter(create_time__contains=post26).order_by('-id')

	elif post27:
		dngred = models.dnguser.objects.filter(update_time__contains=post27).order_by('-id')
