...
def htmlpower(request):
	...
	zd_list = dngadmin_common.dng_ziduan("htmlpower") #获取对应表下所有字段值
	# ----------------------------------------------------------
	#   判断页面权限开始《《《 结束
	# ----------------------------------------------------------
	...
	return render(request,"dngadmin/htmlpower.html",{
...
def htmlpower_menu(request): #访问权限
	# ----------------------------------------------------------
	#   判断页面权限》》》 开始
	# ----------------------------------------------------------
	...
	caidan = models.route.objects.filter(superior_int=0) #查询主菜单
	for caidan1 in caidan:

		if caidan1:
			caidan1_list.append(caidan1)

			candan2 = models.route.objects.filter(superior_int=caidan1.uid_int)
			if candan2:
	...
	cdid = models.route.objects.filter() #查询所有菜单ID
	cdidss = models.route.objects.filter(~Q(superior_int=0))   # 查询所有菜单ID
	looo =[]
	for sk in cdidss:
		looo.append('|'+str(sk.superior_int)+'|')

	zuqxx = models.usergroup.objects.filter(gid_int=qxid).first() #查询对应组的权限
	stre = ""
...
def htmlpower_added(request):  #新增
	# ----------------------------------------------------------
	#   判断页面权限》》》 开始
	# ----------------------------------------------------------
...
def htmlpower_delete(request):  #删除
	# ----------------------------------------------------------
	#   判断页面权限》》》 开始
	# ----------------------------------------------------------
...
def htmlpower_update(request):  #更新修改
	# ----------------------------------------------------------
	#   判断页面权限》》》 开始
	# ----------------------------------------------------------
...
def htmlpower_see(request): #开发者权限
	# ----------------------------------------------------------
	#   判断页面权限》》》 开始
	# ----------------------------------------------------------
...
def htmlpower_post(request):
	...
	if update:
		...
		if fenlei=="menu_text":
			models.usergroup.objects.filter(gid_int=qxid).update(menu_text= keys)
			ok = "yes"
		elif fenlei=="added_text":
			models.usergroup.objects.filter(gid_int=qxid).update(added_text= keys)
			ok = "yes"
		elif fenlei == "delete_text":
			models.usergroup.objects.filter(gid_int=qxid).update(delete_text= keys)
			ok = "yes"
		elif fenlei == "update_text":
			models.usergroup.objects.filter(gid_int=qxid).update(update_text= keys)
			ok = "yes"
		elif fenlei == "see_text":
			models.usergroup.objects.filter(gid_int=qxid).update(see_text= keys)
			ok = "yes"
		else:
			ok ="权限规则错误"
