...
def htmlgroup(request):
	...
	zd_list = dngadmin_common.dng_ziduan("htmlgroup") #获取对应表下所有字段值
	# ----------------------------------------------------------
	#   判断页面权限开始《《《 结束
	# ----------------------------------------------------------
	...
	return render(request,"dngadmin/htmlgroup.html",{
...
def htmlgroup_json(request):
	...
	dngred = models.usergroup.objects.filter().order_by('-id')
...
def htmlgroup_added(request):  #新增
	# ----------------------------------------------------------
	#   判断页面权限》》》 开始
	# ----------------------------------------------------------
	...
	ok = '新增失败'
	off = False
	pws =models.usergroup.objects.filter(gname_str=post2).only('gname_str')
	if not post3:
		post3 = 0
		pid =True
	else:
		pid = models.usergroup.objects.filter(gid_int=post3).only('gid_int')
	if added:
		if post3 =='0':
			user = models.usergroup.objects.filter().order_by('-gid_int').first()
			uid = int(user.gid_int) + 1  # 会员ID递增
			models.usergroup.objects.create(gid_int=uid, gname_str=post2, uperior_int=post3, )
		elif not post2:
			off ='用户组不能为空'
...
def htmlgroup_delete(request):  #删除
	# ----------------------------------------------------------
	#   判断页面权限》》》 开始
	# ----------------------------------------------------------
	...
	if delete: #判断删除权限
		...
		if delete_id: #判断单个删除
			uxxg = models.usergroup.objects.filter(id=delete_id).first()
			udhg = models.user.objects.filter(group_int=uxxg.gid_int).first()
			if udhg:
				off = "用户组下还有用户,不能删除"
				return HttpResponse(off)

			ok = models.usergroup.objects.filter(id=delete_id).delete()
			...
			if "," in id :
				...
				for key in id:
					int(key)
					uxxg = models.usergroup.objects.filter(id=key).first()
					udhg = models.user.objects.filter(group_int=uxxg.gid_int).first()
					if udhg:
						off = "用户组下还有用户,不能删除"
						return HttpResponse(off)

					ok = models.usergroup.objects.filter(id=key).delete()
					if ok:
						ok = 'yes'
					else:
						ok = 'no'
				...
				uxxg = models.usergroup.objects.filter(id=id).first()
				udhg = models.user.objects.filter(group_int=uxxg.gid_int).first()
				...
				ok = models.usergroup.objects.filter(id=id).delete()
				if ok:
					ok = 'yes'
				else:
					ok = 'no'
...
def htmlgroup_update(request):  #更新修改
	# ----------------------------------------------------------
	#   判断页面权限》》》 开始
	# ----------------------------------------------------------
	...
	if update:
		...
		if post1:
			models.usergroup.objects.filter(id=post0).update(gid_int=post1)

		if post2:
			models.usergroup.objects.filter(id=post0).update(gname_str=post2)

		if post3:

			uxxg = models.usergroup.objects.filter(gid_int=post3).first()

			if post3=='0':
				models.usergroup.objects.filter(id=post0).update(uperior_int=post3)
			elif not uxxg :
				off = "没有这个用户组ID"
				return HttpResponse(off)
			elif post3==post1:
		...
		if post4:
			models.usergroup.objects.filter(id=post0).update(integral_int=post4)

		if post5:
			models.usergroup.objects.filter(id=post0).update(money_int=post5)

		if post6:
			models.usergroup.objects.filter(id=post0).update(totalmoney_int=post6)

		if post7:
			models.usergroup.objects.filter(id=post0).update(totalspend_int=post7)

		if post8:
			models.usergroup.objects.filter(id=post0).update(spread_int=post8)

		if post9:
			models.usergroup.objects.filter(id=post0).update(added_int=post9)

		if post10:
			models.usergroup.objects.filter(id=post0).update(look_int=post10)

		if post11:
			models.usergroup.objects.filter(id=post0).update(space_int=post11)

		if post12:
			models.usergroup.objects.filter(id=post0).update(download_int=post12)

		if post13:
			if post13=='on':
				post13=True
				models.usergroup.objects.filter(id=post0).update(trial_bool=post13)
			elif post13=='off':
				post13 = False
				models.usergroup.objects.filter(id=post0).update(trial_bool=post13)
		...
		if post14:
			if post14=='on':
				post14=True
				models.usergroup.objects.filter(id=post0).update(upload_bool=post14)
			elif post14=='off':
				post14 = False
				models.usergroup.objects.filter(id=post0).update(upload_bool=post14)
		...
		if post15:
			if post15=='on':
				post15=True
				models.usergroup.objects.filter(id=post0).update(download_bool=post15)
			elif post15=='off':
				post15 = False
				models.usergroup.objects.filter(id=post0).update(download_bool=post15)
...
def htmlgroup_search(request):  #搜索
	...
	if post1:
		...
		dngred = models.usergroup.objects.filter(gid_int__contains=post1).order_by('-id')

	elif post2:

		dngred = models.usergroup.objects.filter(gname_str__contains=post2).order_by('-id')

	elif post3:

		dngred = models.usergroup.objects.filter(uperior_int__contains=post3).order_by('-id')

	elif post4:

		dngred = models.usergroup.objects.filter(integral_int__contains=post4).order_by('-id')

	elif post5:

		dngred = models.usergroup.objects.filter(money_int__contains=post5).order_by('-id')

	elif post6:

		dngred = models.usergroup.objects.filter(totalmoney_int__contains=post6).order_by('-id')

	elif post7:

		dngred = models.usergroup.objects.filter(totalspend_int__contains=post7).order_by('-id')

	elif post8:

		dngred = models.usergroup.objects.filter(spread_int__contains=post8).order_by('-id')

	elif post9:

		dngred = models.usergroup.objects.filter(added_int__contains=post9).order_by('-id')

	elif post10:

		dngred = models.usergroup.objects.filter(look_int__contains=post10).order_by('-id')

	elif post11:

		dngred = models.usergroup.objects.filter(space_int__contains=post11).order_by('-id')

	elif post12:

		dngred = models.usergroup.objects.filter(download_int__contains=post12).order_by('-id')

	elif post13:

		dngred = models.usergroup.objects.filter(trial_bool__contains=post13).order_by('-id')

	elif post14:

		dngred = models.usergroup.objects.filter(upload_bool__contains=post14).order_by('-id')

	elif post15:

		dngred = models.usergroup.objects.filter(download_bool__contains=post15).order_by('-id')
