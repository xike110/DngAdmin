...
def crudapi(request):
	...
	if api_post:
		if not dngadmin_common.dng_ziduan(api_post) :
			...
			response = HttpResponseRedirect('/dngadmin/crudapi/?jinggao=' + urlstr)
			return response
	...
	return render(request,"dngadmin/crudapi.html",{
...
def crudapi_post(request):
	# ----------------------------------------------------------
	#    通过路径获得栏目ID 》》》开始
	# ----------------------------------------------------------
	return HttpResponse("DngAdmin框架POST预留地址")
