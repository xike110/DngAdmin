...
def sms(request):
	...
	db_values_list = models.sms.objects.filter().values_list()  # 获取数据返回元组列表格式
...
def sms_post(request):
	...
	post1 = request.POST.get('mail_id', '')#短信ID
	
	post2 = request.POST.get('ali_shudanxuan', '')#短信供应商
	
	post3 = request.POST.get('appcode_str', '')#阿里AppCode
	
	post4 = request.POST.get('requests_int', '')#用户每日短信上限
	
	post5 = request.POST.get('youxiao_int', '')#有效时间:秒
	

	post_arry =[post0,post1,post2,post3,post4,post5,]
	...
	if update:
		if post0:
			...
			models.sms.objects.filter(id=post0).update(id=form[0])#ID
	
			models.sms.objects.filter(id=post0).update(mail_id=form[1])#短信ID
	
			models.sms.objects.filter(id=post0).update(ali_shudanxuan=form[2])#短信供应商
	
			models.sms.objects.filter(id=post0).update(appcode_str=form[3])#阿里AppCode
	
			models.sms.objects.filter(id=post0).update(requests_int=form[4])#用户每日短信上限
	
			models.sms.objects.filter(id=post0).update(youxiao_int=form[5])#有效时间:秒
			...
			if form[1]:
				...
				form[1] = 1
			models.sms.objects.create(mail_id=form[1], ali_shudanxuan=form[2], appcode_str=form[3], requests_int=form[4], youxiao_int=form[5], )  #新增数据库
...
@csrf_exempt
def sms_api_json(request):  #api查询
	...
	#   判断页面权限开始《《《 结束
	# ----------------------------------------------------------
	list_arry=models.sms.objects.filter().order_by ("-id")
	gongju = models.sms.objects.count()
