...
def dng_protect():  # 前台安全
    protect = models.protect.objects.filter().order_by('id').first() #最开始的第一条
    return (protect)
...
def dng_yanzheng(user_uid, user_name,user_cookie,user_cookie_echo):  # 验证cookie
    ...
    if not cache.get(user_cookie_echo): #如果缓存没有记录，开始解密重新验证
        anquan =dng_anquan()
        ...
        user = models.dnguser.objects.filter().order_by("-id")
        ifuid = None
        for ua in user:
            #遍历出匹配的ID
        ...
        user = models.dnguser.objects.filter(uid_int=ifuid).first()
        if not user:
            return False
        ...
        return cache.get(user_cookie_echo)#从缓存里取回验证结果
    ...
    if not cache.get(user_cookie_echo):
        ...
        if int(user.uid_int) == int(user_uid) and user.username_str ==user_name and user.token_str==user_token:
            ...
            if user_cookie_echo_rsa==cookie_echo:
                ...
                cache.set(user_cookie_echo, user, 86400*365)  # 在缓存中设置
                return user #返回ID,用户名，COOK记录,验证加密盐
            else:
                return False
        else:
...
def dng_daohang(uid,gm):  # 有权限导航
    ...
    #   @gm   =URL接收管理员切换
    # -------------------------------------------------------------------
    group = dng_usergroup(gid=dng_dnguser(uid).group_int)  # 获取会员组名称
    ...
    if not cache.get('dng_daohang'+str(uid)):
        ...
        for mu_key in group.menu_text:  # 遍历菜单权限
            for key in red2:
                if str(gm) == "1" :
                    cover = models.dngroute.objects.filter(superior_int=key.uid_int, display_bool=True, prove_bool=True).exclude(name_str__icontains="前台").order_by('sort_int')
                elif str(gm) == "2":
                    cover = models.dngroute.objects.filter(superior_int=key.uid_int, display_bool=True,prove_bool=True).exclude(name_str__icontains="后台").order_by('sort_int')
                else:
                    cover = models.dngroute.objects.filter(superior_int=key.uid_int, display_bool=True,prove_bool=True).exclude(name_str__icontains="前台").order_by('sort_int')
                ...
                for li_key in cover:
                    if int(mu_key)==int(li_key.uid_int):
                        ...
                        a_href = "<li><a class=\"J_menuItem\" href=\" "+ li_key.url_str + "\" data-index=\" "+ str(li_key.id) + "\">" + li_key.name_str + "</a></li>"
            ...
            if key.superior_int == 0 and not key.model_str == "cover":
                for mu_key in group.menu_text:  # 遍历菜单权限
                    if int(mu_key)==int(key.uid_int):
                        list_1 = "<li><a class=\"J_menuItem\" href=\"" + key.url_str + "\" data-index=\"" + str(key.id) + "\"><i class=\"" + key.icon_str + "\"></i> <span class=\"nav-label\">" + key.name_str + "</span></a></li>"
                        data_1.append(list_1)

            elif key.superior_int == 0 and key.model_str == "cover":
                list_1 = """<li><a href=\"""" + key.url_str + """\"><i class=\"""" + key.icon_str + """\"></i> <span class="nav-label">""" + key.name_str + """</span><span class="fa arrow"></span></a><ul class="nav nav-second-level collapse"><-二级菜单替换-></ul></li>"""
                data_1.append(list_1)
